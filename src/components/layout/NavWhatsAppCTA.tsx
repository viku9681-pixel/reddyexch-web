...
interface NavWhatsAppCTAProps {
  mobile?: boolean
  phone: string
}

export default function NavWhatsAppCTA({ mobile = false, phone }: NavWhatsAppCTAProps) {
  ...
  useEffect(() => {
    ...
    linkRef.current.href = `https://wa.me/${digits}?text=${message}`
  }, [pathname, digits])
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="bg-red text-white text-sm font-semibold px-4 py-2 rounded-full interactive flex items-center gap-2"
      aria-label="Get Cricket ID on WhatsApp"
    >
